-- Migration: Add multi-tenancy support (FIXED for VARCHAR user IDs)
...
BEGIN
  ...
  LOOP
    -- Generate workspace name and slug
    workspace_name := COALESCE(user_record.full_name, user_record.email) || '''s Workspace';
    workspace_slug := LOWER(REGEXP_REPLACE(COALESCE(user_record.full_name, split_part(user_record.email, '@', 1)), '[^a-z0-9]+', '-', 'g')) || '-' || LEFT(REPLACE(user_record.id::TEXT, '-', ''), 8);
    ...
    -- Only proceed if workspace was created
    IF new_workspace_id IS NOT NULL THEN
      -- Add user as owner
      INSERT INTO workspace_members (user_id, workspace_id, role)
      VALUES (user_record.id, new_workspace_id, 'owner');
