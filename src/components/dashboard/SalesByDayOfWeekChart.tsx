...
interface SalesByDayOfWeekDataPoint {
  day_of_week: string
  sales: number
  percentage: number
}
...
interface SalesByDayOfWeekChartProps {
  data: SalesByDayOfWeekDataPoint[]
  loading?: boolean
  error?: string
  height?: number
  locale?: string
...
/**
 * Sales by day of week chart component displaying sales distribution
 * across weekdays with percentages on top of bars
 */
export function SalesByDayOfWeekChart({
  data = [],
  loading = false,
  error,
  height = 400,
  ...
  const chartData = dayOrder.map(day => {
    ...
    return found || { day_of_week: day, sales: 0, percentage: 0 }
  })
  ...
  const renderCustomLabel = (props: any) => {
    ...
    // Don't show labels for zero percentages to reduce clutter
