  ...
  try {
    ...
    for (const c of constraints) {
      await prisma.$executeRawUnsafe(
        `ALTER TABLE ${c.table_name} DROP CONSTRAINT IF EXISTS ${c.constraint_name}`
      )
    ...
    if (ucmType[0]?.data_type === 'bigint') {
      ...
      await prisma.$executeRawUnsafe(
        `ALTER TABLE user_customer_mappings ALTER COLUMN user_id TYPE TEXT USING user_id::TEXT`
      )
      await prisma.$executeRawUnsafe(
        `ALTER TABLE user_customer_mappings ADD CONSTRAINT user_customer_mappings_user_id_fkey FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE`
      )
    ...
    if (
      awsIdColumns.length > 0 &&
      awsIdColumns[0].data_type !== 'text' &&
      awsIdColumns[0].data_type !== 'character varying'
    ) {
      console.log('\n🔄 Converting users.active_workspace_id to TEXT...')
      await prisma.$executeRawUnsafe(
        `ALTER TABLE users ALTER COLUMN active_workspace_id TYPE TEXT USING active_workspace_id::TEXT`
      )
      console.log('  ✅ Converted')
    ...
    if (!membersExists[0]?.exists) {
      ...
      if (memberUserIdType[0]?.data_type === 'bigint') {
        ...
        await prisma.$executeRawUnsafe(
          `ALTER TABLE workspace_members ADD PRIMARY KEY (user_id, workspace_id)`
        )
        await prisma.$executeRawUnsafe(
    ...
    if (!invitationsExists[0]?.exists) {
      ...
      if (inviteUserIdType[0]?.data_type === 'bigint') {
        ...
        await prisma.$executeRawUnsafe(
          `ALTER TABLE invitations ADD CONSTRAINT invitations_invited_by_id_fkey FOREIGN KEY (invited_by_id) REFERENCES users(id) ON DELETE CASCADE`
        )
